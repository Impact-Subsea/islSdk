...
void XmlSettings::saveDeviceInfo(const Device::Info& info, XmlElementPtr& xml)
{
    if (xml)
    {
        xml->addAttribute("pid", StringUtils::toStr(static_cast<uint_t>(info.pid), 0));
        ...
        xml->addAttribute("build", StringUtils::toStr(info.firmwareBuildNum, 0));
        xml->addAttribute("config", StringUtils::toStr(info.config, 0));
        xml->addAttribute("mode", StringUtils::toStr(info.mode, 0));
        xml->addAttribute("status", StringUtils::toStr(info.status, 0));
    }
}
//--------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
void XmlSettings::saveBias(const Math::Vector3& bias, XmlElementPtr& xml)
{
    if (xml)
    {
...
//--------------------------------------------------------------------------------------------------
bool_t XmlSettings::loadBias(Math::Vector3& bias, const XmlElementPtr& xml)
{
    bool_t ok = false;
...
//--------------------------------------------------------------------------------------------------
void XmlSettings::saveTransform(const Math::Matrix3x3& transform, XmlElementPtr& xml)
{
    if (xml)
    {
        XmlElementPtr node = xml->addElement("transform");
        node->addReal("m00", transform[0][0], 6);
        node->addReal("m01", transform[0][1], 6);
        node->addReal("m02", transform[0][2], 6);
        node->addReal("m10", transform[1][0], 6);
        node->addReal("m11", transform[1][1], 6);
        node->addReal("m12", transform[1][2], 6);
        node->addReal("m20", transform[2][0], 6);
        node->addReal("m21", transform[2][1], 6);
        node->addReal("m22", transform[2][2], 6);
    }
}
//--------------------------------------------------------------------------------------------------
bool_t XmlSettings::loadTransform(Math::Matrix3x3& transform, const XmlElementPtr& xml)
{
    bool_t ok = false;
    ...
    if (xml)
    {
        ...
        if (node)
        {
            transform[0][0] = node->getReal("m00", 0.0);
            transform[0][1] = node->getReal("m01", 0.0);
            transform[0][2] = node->getReal("m02", 0.0);
            transform[1][0] = node->getReal("m10", 0.0);
            transform[1][1] = node->getReal("m11", 0.0);
            transform[1][2] = node->getReal("m12", 0.0);
            transform[2][0] = node->getReal("m20", 0.0);
            transform[2][1] = node->getReal("m21", 0.0);
            transform[2][2] = node->getReal("m22", 0.0);
            ok = true;
        }
    }
