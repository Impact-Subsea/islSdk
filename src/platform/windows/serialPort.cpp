...
std::vector<std::string> SerialPort::getNames()
{
    ...
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"HARDWARE\\DEVICEMAP\\SERIALCOMM", 0, KEY_QUERY_VALUE, &regKey) == ERROR_SUCCESS)
    {
        while (moreData)
        {
            ...
            moreData = RegEnumValueW(regKey, idx, &name[0], &nameSize, nullptr, nullptr, (LPBYTE)&portName[0], &portNameSize) == ERROR_SUCCESS;
            idx++;
...
bool_t SerialPort::open()
{
    ...
    if (MultiByteToWideChar(CP_UTF8, MB_PRECOMPOSED, name.c_str(), -1, &str[4], static_cast<int>(str.size() - 4)))
    {
        ...
        if (handle != INVALID_HANDLE_VALUE)
        {
            ...
            m_threadRxTx = std::thread(&SerialPort::threadRxTx, this);
        }
    }
...
bool_t SerialPort::write(const uint8_t* buf, uint_t size, uint32_t baudrate)
{
    if (m_portHandle != INVALID_HANDLE_VALUE && !m_isSending)
    {
        m_txBuf = buf;
        m_txBufSize = size;
        m_txBaudrate = baudrate ? baudrate : m_baudrate;
        m_isSending = true;
...
//--------------------------------------------------------------------------------------------------
void SerialPort::resetOv(OVERLAPPED& ov)
{
    ov.Internal = 0;
    ov.InternalHigh = 0;
    ov.Offset = 0;
